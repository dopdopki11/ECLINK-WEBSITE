...
const installationGrid = document.querySelector('.installation-grid');

if (installationGrid && Array.isArray(window.ecLinkInstallations)) {
  installationGrid.innerHTML = window.ecLinkInstallations.map((project) => `
    <article class="installation-card is-visible" data-installation="${project.type}">
      <img src="${project.image}" alt="${project.name} ${project.label} installation" loading="lazy" />
      <div><span>${project.label}</span><h3>${project.name}</h3><p>${project.location} · ${project.spec}</p></div>
    </article>
  `).join('');
}

const installationFilters = document.querySelectorAll('[data-installation-filter]');
const installationCards = document.querySelectorAll('[data-installation]');

installationFilters.forEach((button) => {
  button.addEventListener('click', () => {
    const filter = button.dataset.installationFilter;
    installationFilters.forEach((item) => item.classList.toggle('is-active', item === button));
    installationCards.forEach((card) => {
      const matches = filter === 'all' || card.dataset.installation === filter;
